...
$GAMESCategoryn = sanitize($_GET['Category']);
$GAMESCategory = sanitize($_GET['Category']);
$GAMESCategory = takeManuID($GAMESCategory);
...
$totlaGame = TotalGamesBYCAT($GAMESCategory);
$total_pages = ceil($totlaGame['totalgame'] / $num_rec_per_page); 
$start_from = ($page-1) * $total_pages;

if($page > 1){
   $previous = $page - 1;
    echo '<li><a class="skew25" href="'.URL.'/themes/'.THEME.'/games.php?page='.$previous.'&Category='.$GAMESCategoryn.'"><i class="fa fa-angle-left"></i></a></li>';
}
echo '<li><a class="skew25" href="'.URL.'/themes/'.THEME.'/games.php?page=1&Category='.$GAMESCategoryn.'">1</a></li>'; // Goto 1st page  
echo '<li class="selected"><span class="skew25">'.$page.'</span></li>';
			...
			echo '<li><a class="skew25 '.$class.'" href="'.URL.'/themes/'.THEME.'/games.php?page='.$i.'&Category='.$GAMESCategoryn.'">'.$i.'</a></li>'; 
			if($i >= $page+5){
			   break;
			}
}
...
if($page != $total_pages){
   $next = $page + 1;
   echo '<li><a class="skew25" href="'.URL.'/themes/'.THEME.'/games.php?page='.$next.'&Category='.$GAMESCategoryn.'"><i class="fa fa-angle-right"></i></a></li>'; // Goto next page  
}				   
?>
								</div>
								
								<div class="portfolio-items" id="container">
                                <?php $results = GamesCategory($GAMESCategory,$start_from,$num_rec_per_page);
